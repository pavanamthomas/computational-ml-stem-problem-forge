...
| `(X'X)^{-1}X'y` on Hilbert-like X | Finite garbage β, or LAPACK `LinAlgError` | cond(X'X), residual gap | QR / SVD / lstsq | `problems/numerical/ill_conditioned_normal_equations` | Ridge as a different estimand |
